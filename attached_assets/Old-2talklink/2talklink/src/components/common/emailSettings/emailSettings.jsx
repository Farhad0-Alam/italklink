...
const EmailSettings = (props) => {
    ...
    useEffect(() => {
        dispatch(
            setPageHeading({
                pageHeading: "2TalkLink - Email Setting",
                title: "2TalkLink - Email Setting",
            })
        );
    }, [dispatch]);
            ...
            (resp) => {
                if (resp.status === "success") {
                    if (resp.data.emailSettings != "") {
                        ...
                        setMandrillKey(
                            resp.data.emailSettings.mandrillKey
                                ? resp.data.emailSettings.mandrillKey
                                : ""
                        );
                        setSendgridKey(
                            resp.data.emailSettings.sendgridKey
                                ? resp.data.emailSettings.sendgridKey
                                : ""
                        );
                        setSendgridSecret(
                            resp.data.emailSettings.sendgridSecret
                                ? resp.data.emailSettings.sendgridSecret
                                : ""
                        );
                        setSmtpHost(
                            resp.data.emailSettings.smtpHost
                                ? resp.data.emailSettings.smtpHost
                                : ""
                        );
                        setSmtpPort(
                            resp.data.emailSettings.smtpPort
                                ? resp.data.emailSettings.smtpPort
                                : ""
                        );
                        setSmtpUsername(
                            resp.data.emailSettings.smtpUsername
                                ? resp.data.emailSettings.smtpUsername
                                : ""
                        );
                        setSmtpPassword(
                            resp.data.emailSettings.smtpPassword
                                ? resp.data.emailSettings.smtpPassword
                                : ""
                        );
                    }
                        ...
                        <form>
                            ...
                            <div className={styles.profile_box}>
                                <div className={styles.profile_box_body}>
                                    ...
                                    {value == "Mandrill" ? (
                                        <div
                                            className={styles.profile_box_body}>
                                            <div className="pu_input_wrapper_list">
                                                <div className="pu_input_wrapper">
                                                    ...
                                                    <input
                                                        type="text"
                                                        className="pu_input"
                                                        placeholder="Enter Mandrill Key"
                                                        value={mandrillKey}
                                                        onChange={(e) =>
                                                            setMandrillKey(
                                                                e.target.value
