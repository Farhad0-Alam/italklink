...
const Skype = (props) => {
    ...
    const [skypeType, setSkypeType] = useState("");
    const [name, setName] = useState("");
    const [phone, setPhone] = useState("");

    useEffect(() => {
        const data = props.data.sectionData;
        setSkypeType(data.type);
        setName(data.title);
        setPhone(data.phone);
    }, [props]);
    ...
    const updateSms = (e) => {
        if (props.data.sectionData.phone !== phone) {
            ...
            data.sectionData.type = skypeType;
            dispatch(saveSectionACT(data));
        }
    };
    return (
...
export default compose(connect(mapStateToProps, null))(Skype);
