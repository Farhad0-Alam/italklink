...
import { AdminSettings, Coupons, Plans, Users } from "../../../models/DB";
...
routeHandler.login = async (req, res) => {
  ...
  if (!user) {
    res.json({
      status: "error",
      message:
        "Couldn't find a 2TalkLink account associated with this email and password. Please try again.",
    });
  } else {
    if (user.status) {
      let jwtdata = {};
      ...
      res.json({
        status: "error",
        message: `Your 2TalkLink account has been deactivated.`,
      });
    }
  }
};
...
routeHandler.userAccountActive = async (req, res) => {
  ...
  try {
    ...
    let userData = await Users.findOne({ _id: postdata.token }).catch(
      (error) => {
        res.json({
...
routeHandler.getUserById = async (req, res) => {
  try {
    let postdata = req.body;
    const user = await Users.findOne({ _id: postdata.userId }).catch(
      (error) => {
        res.json({
          message: "There was an error!",
          status: "error",
  ...
  if (req.method === "POST") {
    switch (authSlug) {
      case "login":
        ...
        break;
      case "getUserById":
        await routeHandler.getUserById(req, res);
        break;
      case "getPlans":
