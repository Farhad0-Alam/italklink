...
const Users = () => {
      ...
      cell: (row) => (
        <div className="pu_datatable_btns">
          <Tooltip title="Visit" placement="top" arrow>
            <a
              onClick={(e) => router.push(`/admin/users/${row.id}`)}
              className="pu_dt_btn"
            >
              {/* {svg.dt_edit_icon} */}
              Visit
          ...
          <Tooltip title="Assign Plan" placement="top" arrow>
            <a onClick={(e) => assignPlan(e, row)} className="pu_dt_btn ">
              {svg.icon_el_assignPlan}
            </a>
          </Tooltip>
        </div>
      ),
      ...
      cell: (row) => (
        <div style={{ gap: "5px", display: "flex", flexDirection: "row" }}>
          ...
          <button
            ...
            onClick={(e) => {
              planAssign({ planId: row.id, isFree: true });
            }}
          >
            Free
          </button>
  ...
  const planAssign = ({ planId, isFree }) => {
    common.getAPI(
      {
        method: "POST",
        url: "admin/assignPlan",
        data: { planId: planId, user_id: userID, isFree },
      },
      (resp) => {
        if (resp.status === "success") {
          fetchUsers();
        ...
        <div className="pu_datatable_wrapper">
          <div className="pu_pagetitle_wrapper">
            ...
            <div className="pu_pagetitle_right">
              ...
              <button
                className="pu_btn"
                onClick={(e) => setAddUserPopup(!addUserPopup)}
              >
                Add New User
              </button>
            </div>
      ...
      >
        <form onSubmit={addUserFormSubmit}>
          ...
          <div className="pu_input_wrapper">
            ...
            />
            {isEdit ? (
              <div className="pu_input_info">
                <b>Note -</b> Keep the password field blank&#44; if you
