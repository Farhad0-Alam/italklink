...
import {
  ...
  updatePageSeoACT,
  updatePageStyleACT,
} from "../../src/redux/actions/editorAction";
import $ from "jquery";
...
const Settings = (props) => {
  ...
  const activateIconPack = (iconTheme) => {
    const newicon = iconTheme.iconList;
  ...
  const handleTextareaChange = (e, icon) => {
    const data = [...iconLinkList];
    var finditem = data.find((item) => item.id === icon.id);
    finditem.message = e.target.value;
    setIconLinkList(data);
  };
  ...
  const saveSocialIcon = (e, icon) => {
    ...
    if (e.target.value) {
      if (icon.itype === "url") {
        const isURL = validator.isURL(e.target.value, {
  ...
  const selectedModels = planDetails?.selectedModels || [];
    ...
    <>
      <div className={styles.header_settings}>
        <div className={styles.header_settings_wrapper}>
          <span
            onClick={() => editorHeaderBackBtn()}
            className={styles.header_settings_back}
          >
            {svg.back_arrow}
          </span>
          <div className={"pu_container " + styles.pu_container}>
            <div className="pu_tab_nav">
              <ul>
                ...
                <li
                  className={activeTab === "tab_socialsettings" ? "active" : ""}
                >
                  <a onClick={(e) => tabClick("tab_socialsettings")}>
                    Social Settings
                  </a>
                </li>
                {cData.userData.role === 2 ? (
                  <>
                    <li
                      className={activeTab === "tab_template" ? "active" : ""}
                    >
                      <a onClick={(e) => tabClick("tab_template")}>Templates</a>
                    </li>
                    <li
                      className={
                        activeTab === "tab_seosettings" ? "active" : ""
                      }
                    >
                      <a onClick={(e) => tabClick("tab_seosettings")}>
                        SEO Settings
                      </a>
                    </li>
                ...
                <li
                  className={activeTab === "animation_settings" ? "active" : ""}
                >
                  <a onClick={(e) => tabClick("animation_settings")}>
                    Animations Settings
                  </a>
                </li>
              </ul>
        ...
        <div className="pu_setting_wrapper">
          <div className="pu_setting_content">
            ...
            <div
              className={
                "pu_tab_content " +
                (activeTab === "tab_appearance" ? "active" : "")
              }
            >
            ...
            >
              ...
              <div className="pu_setting_content_body">
                <div
                  className={
                    styles.social_icon_wrapper +
                    " " +
                    (cData.userData.role === 1 ? styles.admin_only : "")
                  }
                >
                  <div className={styles.social_pack_list}>
                    {iconPackList
                      .filter((ico) => ico.status === 1)
                      .map((item) => (
                        <div
                          key={item._id}
                          className={
                            styles.social_pack_item +
                            " " +
                            (activeIconPackId === item._id ? styles.active : "")
                          }
                          onClick={() => activateIconPack(item)}
                        >
                          <div className={styles.social_pack_svgs}>
                            {item?.iconList?.map((svg) => (
                              <div
                                className={styles.social_pack_svg_icon}
                                key={svg.id}
            ...
            <SocialIcons
              activeTab={activeTab}
              cData={cData}
              changeSocialIconInput={changeSocialIconInput}
              iconLinkList={iconLinkList}
              setIconLinkList={setIconLinkList}
              saveSocialIcon={saveSocialIcon}
              styles={styles}
              updateSocialIconStatus={updateSocialIconStatus}
              props={props}
            ...
            >
              ...
              <div className="pu_setting_content_body">
                <Alert severity="info">
                  Your appearance settings will be updated when you change your
                  template.
                </Alert>
                <br />
                  ...
                  {templateList.map((temp) => (
                    <div
                      ...
                      className={
                        ...
                        " " +
                        (activeTemplateId === temp._id ? styles.active : "")
                      }
                      onClick={
                        selectedModels?.includes(temp._id)
                          ? () => changeTemplate(temp)
                          : () => {}
                      }
                      style={{
                    ...
                    >
                      {activeTemplateId === temp._id ? (
                        <div className={styles.template_checked}>
                          <span>
                            ...
                            >
                              <path
                                d="M2 12L7.25 17L9.875 14M8 12L13.25 17L22 7M16 7L12.5 11"
                                stroke="white"
                                strokeWidth="2"
                                strokeLinecap="round"
                      ...
                      <div className={styles.template_icon}>
                        {temp.thumb?.url ? (
                          ...
                          <span className={styles.template_icon_svg}>
                            {svg.DFY_temp_icon}
                          </span>
                        )}
                      </div>
                      {cData.userData?.role !== 1 &&
                        !selectedModels?.includes(temp._id) && (
                          <div className={styles.template_lock}>
                            <Image
                              height={100}
                              width={100}
                              src="/lock.svg"
              ...
              <div className="pu_setting_content_body">
                <div className={styles.seo_wrapper}>
                    ...
                    <div className={styles.seo_pagelist}>
                      {pageList.map((page) => (
                        <div
                          key={page._id}
                          className={
                            styles.seo_page_item + " " + isActiveSeo(page._id)
                          }
                  ...
                  <div className={styles.seo_settings}>
                        ...
                        <div className="pu_tags">
                          {seoTagList.map((tag, index) => (
                            <Chip
                              ...
                              label={tag}
                              variant="outlined"
                              onDelete={() => handleSeoTagDelete(index)}
                            />
                          ))}
                        </div>
                        <form onSubmit={(e) => addSeoTag(e)}>
                          <input
                            className="pu_tag_input"
                            type="text"
                    ...
                    <button className="pu_btn" onClick={(e) => saveSeoTag(e)}>
                      Save Settings
                    </button>
                  </div>
                </div>
            ...
            <div
              className={
                "pu_tab_content " +
                (activeTab === "tab_analytics" ? "active" : "")
              }
            >
              <div className="pu_setting_content_header">
                <h3>Analytics</h3>
              </div>
              <div className="pu_setting_content_body"></div>
            </div>
            {/* Analytics tab end */}
            ...
            <div
              className={
                "pu_tab_content " +
                (activeTab === "tab_integration" ? "active" : "")
              }
            >
              <div className="pu_setting_content_header">
                <h3>Integration</h3>
            ...
            <div
              className={
                "pu_tab_content " +
                (activeTab === "tab_domainsettings" ? "active" : "")
              }
            >
              <div className="pu_setting_content_header">
            ...
            <div
              className={
                "pu_tab_content " +
                (activeTab === "animation_settings" ? "active" : "")
              }
            >
              <div className="pu_setting_content_header">
              ...
              <div className="pu_setting_content_body">
                <AnimationSetting
                  data={cData.editor.sectionList}
                  page={cData.editor.pageData}
                />
              </div>
            </div>
