...
function Images(props) {
  ...
  useEffect(() => {
    setNoOfImages(props.data.sectionData?.noOfImages);
    setTitle(props.data.sectionData?.title);
  }, []);
  ...
  useEffect(() => {
    if (props.data.otherBusiness.length != 0) {
      ...
      [1, 2, 3, 4].forEach((val, i) => {
        if (i < index) {
          imageCount[i] = {
            ...props.data.otherBusiness[i],
            image: process.env.s3URL + props.data.otherBusiness[i].image,
            id: props.data._id + i,
          };
  ...
  const handleType = (val) => {
    ...
    data.sectionData.noOfImages = val;
      ...
      if (url) {
        setTimeout(() => {
          const image = document.getElementById(
            "cropper_image" + props.data._id + i
          );
  ...
  const handleUpdateTitle = (e) => {
    if (props.data.sectionData.title !== e.target.value) {
      const data = { ...props.data };
  ...
  const uploadImageButton = (idToFind) => {
    if (urlList[idToFind] == undefined) {
      ...
      const URlEmpty = validator.isEmpty(urlList[idToFind], {
        ignore_whitespace: true,
      });
      const isURL = validator.isURL(urlList[idToFind], {
    ...
    <>
      <div className="pu_input_wrapper">
        ...
        <div className={styles.full_grid_flex}>
          <div
            className={`${styles.boxes_wrap} ${
              noOfImages === "Full" && styles.active
            }`}
            onClick={() => handleType("Full")}
          >
            <div className={styles.outer_border}>
              <div className={styles.single_image_box}>{svg["imageIcon"]}</div>
            </div>
            Full
          </div>
          <div
            className={`${styles.boxes_wrap} ${
              noOfImages === "Split" && styles.active
            }`}
            onClick={() => handleType("Split")}
          >
            <div className={`${styles.outer_border}  ${styles.split}`}>
              {[1, 2].map((val) => (
                <div key={val} className={styles.single_image_box}>
                  {svg["imageIcon"]}
                </div>
              ))}
            </div>
          ...
          <div
            className={`${styles.boxes_wrap} ${
              noOfImages === "Grid" && styles.active
            }`}
            onClick={() => handleType("Grid")}
          >
            <div className={`${styles.outer_border}  ${styles.grid}`}>
              {[1, 2].map((val) => (
                <div className={styles.quard_image_box_grid} key={val}>
      ...
      {imageCount
        .slice(
          0,
          noOfImages === "Grid" ? undefined : noOfImages === "Split" ? 2 : 1
        )
        .map((val, i) => (
          <div
            className={`pu_input_wrapper_list  ${styles.list_wrap}`}
            key={props.data._id + i}
          >
            <div
              className={`pu_input_wrapper ${styles.url_box}`}
              style={{ flex: 2 }}
            >
              <input
                type="text"
                className="pu_input"
                placeholder="Enter the redirect URL for this image."
                name={props.data._id + i}
                value={
                  urlList[props.data._id + i] ? urlList[props.data._id + i] : ""
                }
                onChange={(e) => {
                  UrlHander(e, i);
                }}
            ...
            <div className={styles.image_box_wrap}>
              ...
              <div className="text-center" style={{ marginBottom: "25px" }}>
                <button
                  type="submit"
                  className="pu_btn"
                  onClick={() => uploadImageButton(props.data._id + i)}
                >
                  Upload
                </button>
              </div>
      ...
      >
        <div className="pu_image_cropper">
          <img
            id={"cropper_image" + props.data._id + copperID}
            src={imageURL}
            alt=""
            style={{ width: "100%" }}
          />
