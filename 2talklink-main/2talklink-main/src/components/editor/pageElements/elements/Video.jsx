...
function Video(props) {
  ...
  useEffect(() => {
    if (props.data.sectionData.title.split(" ")[0] == "Vimeo") {
      ...
      setTitle(props.data.sectionData.dynamicTitle);
    } else {
  ...
  const handleUploadVideo = () => {
    ...
    if (videoType == "YouTube") {
      ...
      let isVAlid =
        youTubeVideo.includes("youtube.com") ||
        youTubeVideo.includes("youtu.be");
      if (!isVAlid) {
    ...
    if (videoType == "Vimeo") {
      ...
      let isVAlidVimeo =
        vimeoVideo.includes("vimeo.com") ||
        vimeoVideo.includes("player.vimeo.com");
      if (!isVAlidVimeo) {
  ...
  const handleUpdateTitle = (e) => {
    if (props.data.sectionData.dynamicTitle !== e.target.value) {
      const data = { ...props.data };
      data.sectionData.dynamicTitle = e.target.value;
      dispatch(saveSectionACT(data));
    }
  };
      ...
      <div className={style.vedio_element_btn_wrapper}>
        <div
          className={`${style.btn_box}  ${
            videoType == "YouTube" && style.active_btn
          }`}
          onClick={() => setVideoType("YouTube")}
        >
          {svg.youTubeIcon} YouTube
        </div>
        <div
          className={`${style.btn_box}  ${
            videoType == "Vimeo" && style.active_btn
          }`}
