...
function QRCode(props) {
  ...
  useEffect(() => {
    const isURL = validator.isURL(props?.data?.sectionData?.url || "", {
      protocols: ["http", "https", "ftp"],
      require_protocol: false,
    });
    isURL && setQrURl(props.data?.sectionData?.url);
    setQrTitle(props.data.sectionData?.title);
  }, []);

  const generateQR = (e) => {
    e.preventDefault();
    const URlEmpty = validator.isEmpty(QrUrl || "", {
      ignore_whitespace: true,
    });
    const isURL = validator.isURL(QrUrl || "", {
      protocols: ["http", "https", "ftp"],
      require_protocol: false,
    });
    if (URlEmpty) {
    ...
    if (props?.data?.sectionData?.url !== QrUrl) {
      const data = { ...props.data };
      data.sectionData.url = QrUrl;
      dispatch(saveSectionACT(data));
    }
  };

  const updateTitle = () => {
    if (props?.data?.sectionData?.title !== qrTitle) {
      const data = { ...props.data };
      data.sectionData.title = qrTitle;
      dispatch(saveSectionACT(data));
    }
  };
  return (
