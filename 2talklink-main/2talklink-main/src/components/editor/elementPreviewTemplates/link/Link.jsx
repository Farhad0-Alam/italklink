...
import { setFillColor } from "../../../../../utils/setFillColor";
import { common } from "../../../../helper/Common";
import styles from "./Link.module.css";
const LinkElement = (props) => {
  const { template } = props?.props || {};
  const { templateStyle } = template || {};
  const { textfont } = templateStyle || {};
  ...
  const linkClick = (url) => {
    ...
    if (props.item.sectionData.pageType === "link") {
  ...
  // Icons Extracting
  let icon =
    props.item.sectionData.icon?.type && props.item.sectionData.icon.icon;

  if (props.item.sectionData.icon?.type) {
    const secondaryColor = props.secondaryColor;
    const defaultGradient = props.defaultGradient;
    const primaryColor = props.primaryColor;
    switch (props.htmlId) {
      case "1": {
        icon = setFillColor(icon, extractGradientColor(defaultGradient));
        break;
      }
      case "2": {
        icon = setFillColor(icon, primaryColor);
        break;
      }
      case "3": {
        icon = setFillColor(icon, secondaryColor);
        break;
      }
      case "4": {
        icon = setFillColor(icon, secondaryColor);
      ...
      case "7": {
        icon = setFillColor(icon, secondaryColor);
        // icon = setFillColor(icon, "#9D9D9D");
        break;
      }
      case "8": {
        icon = icon?.replace(`height=30px`, `height=20px`);
        icon = icon?.replace(`width=30px`, `width=20px`);
        icon = setFillColor(icon, secondaryColor);
        icon = setFillColor(icon, "#fff");
        break;
      }
      case "9": {
        icon = icon?.replace(`height=30px`, `height=20px`);
      ...
      case "13": {
        icon = icon?.replace(`fill=#111`, "fill=#00000061");
        // icon = icon?.replace(`fill=#111`, `fill=${secondaryColor}`);
        const wrapper = `<span  style="background-color: ${primaryColor};" class="th-13-icon">${icon}</span>`;
        icon = wrapper;
      }
      case "14": {
        icon = setFillColor(icon, primaryColor);
      ...
      case "18": {
        icon = setFillColor(icon, defaultGradient);
        break;
      }
      case "19": {
        icon = setFillColor(icon, primaryColor);
      ...
      case "21": {
        icon = setFillColor(icon, templateStyle?.tertiary_color || "#fff");
        break;
      }
      case "22": {
        icon = setFillColor(icon, primaryColor);
  ...
  if (props.htmlId == "1") {
    return (
      <>
        ...
        >
          {props.item.sectionData.pageType === "link" ? (
            <>
              <a
                target={props.item.sectionData.openNewTab ? "_blank" : "_self"}
                href={
                  props.item.sectionData.type !== "link" &&
                  props.item.sectionData.fullURL
                }
                onClick={
                  props.item.sectionData.type === "link"
                    ? () => linkClick(props.item.sectionData.fullURL)
                    : null
                }
                ...
                {icon ? (
                  ...
                  <span
                    style={
                      icon
                        ? {
                            color: extractGradientColor(props.defaultGradient),
                          }
                        : {
            ...
            <>
              <div
                onClick={() =>
                  linkClick(
                    (props.isAdmin === true
                      ? "/preview/" + props.tempId
                      : props.linkSlug) +
                      "/" +
                      props.item.sectionData.pageSlug
                  )
                }
              >
                <a
                  target={
                    props.item.sectionData.openNewTab === true
                      ? "_blank"
                      : "_self"
                  }
      ...
      <>
        ...
        >
          {props.item.sectionData.pageType === "link" ? (
            <a
              target={
                props.item.sectionData.openNewTab === true ? "_blank" : "_self"
              }
              href={
                props.item.sectionData.type !== "link" &&
                props.item.sectionData.fullURL
        ...
        >
          {props.item.sectionData.pageType === "link" ? (
            <>
              ...
              >
                {!icon && (
                  <span className={styles.svg__icons}>
                    {" "}
                    <svg
                      width="20"
                      height="20"
    ...
    const template = (
      <div
        className={
          ...
          (props.isEditorPreview === true ? styles.editor_link : "") +
          "  " +
          styles["wrapper_" + props.item.sectionData.type] +
          " " +
          (props.item?.animation
            ? `animate__animated animate__${
                props.item?.animation?.animationRepeat?.length == 1
      ...
      >
        {props.item.sectionData.pageType === "link" ? (
          <a
            ...
            style={{ color: templateStyle?.tertiary_color || "#fff" }}
          >
            {icon ? (
              <div
              ...
              >
                ...
                <div style={{ marginLeft: "auto" }}>
                  <>
                    {props.item.sectionData.type === "location" ? (
                      <>
                        {props.item.sectionData.url ||
                          props.item.sectionData.linkTitle}
                      </>
                    ) : (
                      <>
    ...
    switch (props.item.sectionData.type) {
      case "link":
        return <div style={{ order: -1 }}>{template}</div>;
      case "location":
        return (
          <>
            <h1
        ...
        >
          {props.item.sectionData.pageType === "link" ? (
            <a
              ...
              style={{ color: props.secondaryColor, fontFamily: textfont }}
            >
              {icon ? (
                <div
                  style={{
                ...
                >
                  <div>
                    <span
                      className={styles["themes_" + props.htmlId + "_icon"]}
                      dangerouslySetInnerHTML={{
                        __html: icon,
                      }}
                    ></span>
