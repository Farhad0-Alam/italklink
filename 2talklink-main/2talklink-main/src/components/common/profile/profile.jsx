...
const Profile = (props) => {
  ...
  useEffect(() => {
    dispatch(
      setPageHeading({
        pageHeading: "2TalkLink - Profile",
        title: "2TalkLink - Profile",
      })
    );
  }, [dispatch]);
  ...
  const [clickedRenew, setClickedRenew] = useState(false);

  const handlePlanRenew = (data) => {
      ...
      <div className="pu_container">
        ...
        <div className={styles.profile_wrapper}>
          <div className={styles.profile_left}>
            ...
            {currentPlan?.isFreePlanExpired &&
              currentPlan?.isFreePlan === false && (
                <>
                  <div className="pu_plan_detail_wrapper">
                    <span className={"pu_plan_tagline_inactive"}>
                      Expired Free Plan
                    </span>
                    <div className="pu_plan_status">
                      ...
                      <div className="pu_plan_detail">
                        ...
                        <p>
                          Expiry Date :{" "}
                          {common.dateFormatWithoutTime(
                            currentPlan?.freeTrialValidityDate
                          )}
                        </p>
                        <div>
                          <p className="plan-price">
                            Price: {currentPlan?.planData?.currency?.symbol}
                            {currentPlan?.planData?.price}
                          </p>
                          <button
                            className="pu_btn"
                            disabled={clickedRenew}
                            onClick={() => {
                              !clickedRenew &&
                                handlePlanRenew({
                                  planId: currentPlan.planData._id,
                                });
                            }}
                          >
                            Upgrade Plan
            ...
            {!currentPlan?.isFreePlanExpired &&
              currentPlan?.isFreePlan === true && (
                <>
                  <div className="pu_plan_detail_wrapper">
                    <span className={"pu_plan_tagline_active"}>
                      Active Free Plan
                    </span>
                    <div className="pu_plan_status">
                      <div className="pu_plan_icon">{svg.price_tag}</div>
                    ...
                    <span
                      className={
                        currentPlan.isExpired
                          ? "pu_plan_tagline_inactive"
                          : "pu_plan_tagline_active"
                      }
                    >
                      {currentPlan.isExpired ? "Expired Plan" : "Active Plan"}
                    </span>
                    <div className="pu_plan_status">
                      <div className="pu_plan_icon">{svg.price_tag}</div>
                      <div className="pu_plan_detail">
