...
routeHandler.updateIconData = async (req, res) => {
    let postdata = req.body;
    let user = req.vsuser;

    try {
        // প্রয়োজনীয় ফিল্ড যাচাই করা
        let validateFields = ["template_id", "icon_data"];
        ...
        if (response.status) {
            ...
            // টেমপ্লেট খুঁজে বের করা
            let templateData = await templateTbl.findOne({ _id: postdata.template_id });
            if (templateData && typeof templateData._id != 'undefined') {
                // SocialIconData আপডেট করা হচ্ছে
                let set = {
                    SocialIconData: postdata.icon_data // নতুন icon_data সেট করা
                };

                await templateTbl.updateOne({ _id: postdata.template_id }, { $set: set });

                return res.json({
                    status: "success",
                    message: "SocialIconData updated successfully.",
                });
            } else {
                return res.json({
                    status: "error",
            ...
            return res.json({
                status: "error",
                message: "Validation failed. Please provide the required fields.",
            });
        }
    } catch (err) {
        return res.json({
	...
	if (req.method === 'POST') {
		switch (editorSlug) {
			...
			case 'updateSocialIconsPack':
				...
				break;
			case 'updateIconData':
				await routeHandler.updateIconData(req, res)
				break;
