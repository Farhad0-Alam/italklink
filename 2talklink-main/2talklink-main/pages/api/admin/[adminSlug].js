...
routeHandler.getUser = async (req, res) => {
  ...
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      let user = await Users.findById(postdata?.id).catch((error) => {
        res.json({
          error: error,
          message: "There was an error!",
          status: "error",
  ...
  try {
    ...
    if (ordersave) {
      if (userdata) {
        ...
        // If the user is purchasing a free trial
        if (postdata?.isFree) {
          const newFreeTrialDate = addDays(currentDate, plandata.testDays);
          await Users.findOneAndUpdate(
            { _id: userdata._id },
            {
              planName: plandata.planname,
              plan: postdata?.planId,
              freeTrialValidityDate: newFreeTrialDate,
              totalPlanCardCount: plandata.numberOfEcard,
              isFreePlan: true,
              $unset: { validityDate: "" },
            }
          );
          ...
          await Users.findOneAndUpdate(
            { _id: userdata._id },
            {
              ...
              isFreePlan: false,
              totalPlanCardCount:
                userdata?.totalPlanCardCount || 0 + plandata.numberOfEcard,
    ...
    return res.json({
      status: "success",
      message: "Plan has successfully been assigned to the user.",
      data: "",
    });
  } catch (err) {
    // console.log(err);
  ...
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      ...
      let missingFields = requiredFields.filter(
        (field) => !postdata[field] || postdata[field].length === 0
      );
      ...
      if (postdata.id) {
        ...
        let set = {
          ...
          selectedModels: postdata.selectedModels,
          validity: postdata.validity,
          currency: postdata.currency,
          price: postdata.price,
          numberOfEcard: postdata.numberOfEcard,
          testDays: postdata.testDays || 0,
          customSelection: postdata.customSelection || false,
          customFields: postdata.customFields || [],
          features: postdata.features || [],
        };

        await Plans.updateOne(where, { $set: set })
          .then(() => {
            res.json({
              status: "success",
          ...
          .catch((err) => {
            res.json({
              status: "error",
              message: "Error updating plan: " + err.message,
            });
          });
      } else {
        // Create new plan
        let where = { planname: postdata.planname };
        let planExists = await Plans.findOne(where);

        if (planExists) {
          return res.json({
            status: "error",
            message: "The plan already exists. Please use another plan name.",
          });
        }
...
routeHandler.getPlanById = async (req, res) => {

  const { planId } = req.body; // Assuming plan ID is passed in the URL
...
routeHandler.getAllTemplates = async (req, res) => {
  try {
    ...
    let totalTemplates = await Templates.find().countDocuments();
...
routeHandler.updateSocialTypeStatus = async (req, res) => {
  ...
  try {
    ...
    if (response.status) {
      ...
      let socialData = await SocialType.findOne(where);
      if (socialData && typeof socialData._id != "undefined") {
        const findAllSocialPacks = await SocialPack.find().lean();
        findAllSocialPacks?.forEach(async (icon) => {
          const updatedIconList = icon.iconList.map((icon) => {
            if (icon?.id === typeId) {
              return {
                ...icon,
                status: typeStatus,
          ...
          await SocialPack.updateOne(
            { _id: new mongoose.Types.ObjectId(icon?._id) },
            { $set: { iconList: updatedIconList } }
          );
        });
...
routeHandler.deleteSocialType = async (req, res) => {
  ...
  try {
    ...
    const findAllSocialPacks = await SocialPack.find().lean();
    findAllSocialPacks?.forEach(async (icon) => {
      const updatedIconList = icon.iconList.filter(
        (icon) => icon.id !== typeId
      );
      await SocialPack.updateOne(
        { _id: new mongoose.Types.ObjectId(icon?._id) },
    ...
    const findAllCampaigns = await campaignModal.find().lean();
    findAllCampaigns?.forEach(async (campaign) => {
      const updatedCampaignIconList = campaign.SocialIconData.filter(
        (icon) => icon.id !== typeId
      );
      await campaignModal.updateOne(
        { _id: new mongoose.Types.ObjectId(campaign?._id) },
        { $set: { SocialIconData: updatedCampaignIconList } }
      );
    });
  ...
  if (req.method === "POST") {
    switch (adminSlug) {
      ...
      case "getUser":
        ...
        break;
      case "getPlanById":
        await routeHandler.getPlanById(req, res);
        break;
      case "addCoupon":
      ...
      case "getAllTemplates":
        await routeHandler.getAllTemplates(req, res);
        break;
      case "saveTemplate":
