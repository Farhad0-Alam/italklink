    ...
    if (templateData) {
      ...
      if (!postdata.action && postdata.action !== "previewPage") {
        if (check.isEnabled == true) {
          if (expiredChecking(checkExpire?.validityDate)) {
          ...
          if (expiredChecking(checkExpire?.freeTrialValidityDate)) {
            if (check.isAdEnabled == false) {
              return res.json({
                status: "success",
                message: "",
      ...
      if (templatePageData && typeof templatePageData._id != "undefined") {
        let pageSections = await templateSectionTbl
          ...
          .sort({ sort: 1 });
        if (
          ["23", "24"].includes(templateData.html_theme_id) &&
          where2.slug !== "home"
        ) {
          const pageData = await templateSectionTbl.find({
            templateId: templateData._id,
            "sectionData.pageType": "page",
          });
          pageSections.push(...pageData);
        }
        if (
        ...
        res.json({
          ...
          data: {
            ...
            freeTrialValidityDate: checkExpire?.freeTrialValidityDate,
          },
        });
      } else {
        res.json({
